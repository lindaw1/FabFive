<?php
    ...
    foreach($arrayPackageObjs as $row){
        ...
        echo' <p class="cost oldCost" >C$ <span class="oldPrice" style="text-decoration: line-through;">'.$row->getPkgName().'</span> </>';
        echo' <p class="cost saleCost">C$ <span class="newPrice">' . $row->getPkgName() .'</span> </p>';
        echo' <p class="cost perPerson">per person</p>
                </div>

                <div class="textBoard">
                <h1 class="whiteText salePackageTitle">'.$row->getPkgName().'</h1>';
        echo'   <p class=" whiteText subDescription">'.$row->getPkgName().'</p>
        ...
        echo'    <p class="whiteText packageTimeframe"> Guaranteed Departure:  
                    <span class="startDate">'.$row->getPkgName().'</span> to <span class="endDate">'.$row->getPkgName().'</spanclass> 
                </p>
                <p class="whiteText packageDescription">'.$row->getPkgName().'
                </p>
